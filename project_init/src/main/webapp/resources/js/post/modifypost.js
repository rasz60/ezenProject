...
function setModifyPost(data, dateCount) {
	...
	// postDt Setting
	var postDt = $('div.planDt');
	
	if ( postDt.children('input').length != 0 ) {
		var index = postDt.children('input').length / 2;
		...
		for ( var i = 0; i < Number(index); i++ ) {
			var planDtNum = postDt.children('input[name=planDtNum]').eq(i).val();
			var placeName = postDt.children('input[name=placeName]').eq(i).val();
			...
			dtNums.push(planDtNum);
			
			for ( var j = 0; j < $('.addLocBtn.active').length; j++ ) {
				if ( $('.addLocBtn.active').eq(j).attr('data-index') == planDtNum ) {
					$('.addLocBtn.active').eq(j).attr('disabled', true);
				}
			}
		}
	}
...
$(document).ready(function() {
	...
	$(document).on('click', '.delLocBtn', function() {
		...
		for ( var i = 0; i < dtNums.length; i++ ) {
			if ( index == dtNums[i] ) {
				var delDtNum = $('input[name=delDtNum]').val();
